...
class plotSnow(makeMongoDatabase):
    ...
    def set_up_grid(self):
        ...
        self.df.rename(columns={'id_num': '_id',
                                'land_sea_value': 'land_sea'},
                       inplace=True)
        m = self.make_map(proj='merc')
        self.df['x'], self.df['y'] = m(self.df['lon'].values,
                                       self.df['lat'].values)
        cols = ['_id', 'area', 'lat', 'lon', 'x', 'y', 'land_sea']
        self.df = self.df.loc[:, cols]
        self.region_area = self.df['area'].sum()
        logging.debug('total area of region: {}'.format(self.region_area))
        min_x = self.df['x'].min()
        max_x = self.df['x'].max()
        min_y = self.df['y'].min()
        max_y = self.df['y'].max()
        self.grid_x, self.grid_y = np.mgrid[min_x: max_x: 3000j,
                                            min_y: max_y: 3000j]
        snow = '#FEFEFE'
    ...
    def make_plots_from_collections(self,
        ...
        This also creates a time series dataframe.
        if SHOW and SAVE=FALSE, this will only create timeseries"""
        plt.ioff()
        ...
        for year in YEARS:
            ...
            date_range = map(lambda date: date.strftime('%Y-%m-%d'),
                             date_range)
    ...
    def append_coverage(self, date_str, contour_values):
        ...
        logging.debug('snow coverage on {0}: '
                      '{1} km^2'.format(date_str, coverage))
    ...
    def make_plot_from_contour_values(self,
                                      date_str,
                                      contour_values,
                                      SHOW,
                                      SAVE):
        ...
        grid_z0 = griddata(self.df[['x', 'y']].values,
                           contour_values,
